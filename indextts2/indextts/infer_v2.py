...
# Respect existing cache settings set by the host application; only set a default
# if nothing has been configured yet.
if 'HF_HUB_CACHE' not in os.environ and 'HF_HOME' not in os.environ and 'TRANSFORMERS_CACHE' not in os.environ:
...
class IndexTTS2:
    def __init__(
        ...
        # Determine a consistent HuggingFace cache directory
        self.hf_cache_dir = (
            os.environ.get('HF_HOME')
            or os.environ.get('HF_HUB_CACHE')
            or os.environ.get('TRANSFORMERS_CACHE')
            or os.path.join(self.model_dir, 'cache')
        )
        ...
        # Try to load from a concrete local snapshot path to avoid cache layout mismatches
        repo_id_w2v = "facebook/w2v-bert-2.0"
        local_repo_path = None
        # Prefer old-style cache layout if present
        try:
            if self.hf_cache_dir:
                models_dir = os.path.join(self.hf_cache_dir, f"models--{repo_id_w2v.replace('/', '--')}", "snapshots")
                if os.path.isdir(models_dir):
                    snapshots = [p for p in glob.glob(os.path.join(models_dir, '*')) if os.path.isdir(p)]
                    if snapshots:
                        # Pick the most recently modified snapshot folder
                        snapshots.sort(key=lambda p: os.path.getmtime(p), reverse=True)
                        local_repo_path = snapshots[0]
        except Exception:
            local_repo_path = None

        if local_repo_path is None:
            try:
                local_repo_path = snapshot_download(repo_id=repo_id_w2v, cache_dir=self.hf_cache_dir)
            except Exception:
                local_repo_path = None

        if local_repo_path is not None:
            self.extract_features = SeamlessM4TFeatureExtractor.from_pretrained(local_repo_path, local_files_only=True)
        else:
            self.extract_features = SeamlessM4TFeatureExtractor.from_pretrained(
                repo_id_w2v, cache_dir=self.hf_cache_dir
            )
        ...
        semantic_code_ckpt = hf_hub_download(
            "amphion/MaskGCT", filename="semantic_codec/model.safetensors", cache_dir=self.hf_cache_dir
        )
        ...
        campplus_ckpt_path = hf_hub_download(
            "funasr/campplus", filename="campplus_cn_common.bin", cache_dir=self.hf_cache_dir
        )
