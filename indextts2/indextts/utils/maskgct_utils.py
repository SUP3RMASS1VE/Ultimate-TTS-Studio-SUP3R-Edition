...
def build_semantic_model(path_='./models/tts/maskgct/ckpt/wav2vec2bert_stats.pt'):
    # Use host-defined cache directory if available
    cache_dir = (
    ...
    try:
        semantic_model = Wav2Vec2BertModel.from_pretrained("facebook/w2v-bert-2.0", cache_dir=cache_dir)
    except Exception:
        # Fallback: download a full local snapshot and load from that folder
        local_repo_path = snapshot_download(repo_id="facebook/w2v-bert-2.0", cache_dir=cache_dir, local_files_only=False)
        semantic_model = Wav2Vec2BertModel.from_pretrained(local_repo_path, local_files_only=True)
    semantic_model.eval()
